...
int main (int argc, char **argv)
{
  ...
  signal(SIGCHLD, handle_sigchld); //quando figli cambiano stato
...
void handle_sigchld(int x) {
  ...
  while (waitpid(-1, 0, WNOHANG) > 0) {} //-1 signfica qualsiasi figlio
  errno = saved_errno;
}
//se io disabilito gestione dei figli succede defunct, ovvero un processo che termina senza che nessuno raccolga l'exit status, quindi strutture dati rimangono allocate nel kernel, quindi zombie. NON VA BENE
